...
public class EntityMonitor<T: NSManagedObject where T: CoreDataModelable, T: Hashable> {
  ...
  public init(context: NSManagedObjectContext, frequency: FireFrequency = .OnSave, filterPredicate: NSPredicate? = nil) {
    ...
    self.entityPredicate = NSPredicate(format: "entity == %@", T.entityDescription(in: context))
  }
...
private class BaseEntityMonitorDelegate<T: NSManagedObject where T: CoreDataModelable, T: Hashable>: NSObject {
  ...
  @objc final func evaluateChangeNotification(_ notification: NSNotification) {
    ...
    owner.context.performAndWait { [predicate = owner.combinedPredicate] in
      func process(value: NSSet?) -> EntitySet {
        return value.flatMap { $0.filtered(using: predicate) as? EntitySet } ?? []
      }

      let inserted = process(value: changeSet[NSInsertedObjectsKey as NSObject] as? NSSet)
      let deleted = process(value: changeSet[NSDeletedObjectsKey as NSObject] as? NSSet)
      let updated = process(value: changeSet[NSUpdatedObjectsKey as NSObject] as? NSSet)
...
private final class ForwardingEntityMonitorDelegate<Delegate: EntityMonitorDelegate>: BaseEntityMonitorDelegate<Delegate.T> {
  ...
  override func handleChanges(inserted: EntitySet, deleted: EntitySet, updated: EntitySet) {
    ...
    if !inserted.isEmpty {
      delegate.entityMonitorObservedInserts(monitor: owner, entities: inserted)
    }

    if !deleted.isEmpty {
      delegate.entityMonitorObservedDeletions(monitor: owner, entities: deleted)
    }

    if !updated.isEmpty {
      delegate.entityMonitorObservedModifications(monitor: owner, entities: updated)
    }
  }
}
